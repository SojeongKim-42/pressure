...
Dataset은 hand/object mesh annotation의 정확도를 기준으로 DexYCB를 사용한다. 현재는 static holding만 가정하여 가속도 a ≈ 0으로 둔다. Support case (손이 물체를 아래에서 받치는 경우)는 현재 formulation에서 n_k와 중력 방향이 평행해질 때 t_k가 undefined가 되는 singularity 문제가 있으므로 scope에서 제외한다. Friction은 1D로 가정하여 t_k를 중력 반대 방향 벡터를 contact surface tangential plane에 projection한 방향으로 정의한다.
...
Minimum L2 norm은 F_rnn = 0으로 두는 것과 수학적으로 동치다. 즉 "실제 인간의 grasp behavior prior"가 전혀 없는 formulation이다. 실제 인간은 필요한 force보다 더 크게 쥐는 경향이 있으므로 이 방법은 systematic underestimation에 가까울 수 있다. 이를 임의의 scaling factor로 수정하는 것은 근거가 없으므로, limitation으로 명시하고 이후 OPENTOUCH validation에서 실제 distribution과 비교하여 data-driven correction을 검토한다. 또한 1D friction 가정은 torque balance에 필요한 lateral friction을 표현하지 못해 일부 grasp에서 infeasibility를 유발할 수 있으며, 이는 추후 2D friction 확장으로 해결 가능하다.
