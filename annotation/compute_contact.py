...
# setdefault라서 이미 환경변수가 있으면 덮어쓰지 않는다.
...
# [주의] little과 ring 순서가 직관과 다름. _PART_TO_FINGER는 16개 part(관절)를
#   6개 손가락 라벨(0=palm..5=thumb)로 매핑한다. 각 vertex는 skinning weight가
#   가장 큰 part에 속한 것으로 보고 이 표로 손가락을 정한다(load_hand 참고).
...
_NON_CONTACT_COLOR = np.array([190, 190, 190, 255], dtype=np.uint8)  # 비접촉 vertex 회색
...
def load_hand(sample, label):
  ...
  mano_layer = ManoLayer(flat_hand_mean=False,
                         ...
                         use_pca=True)
  betas = torch.tensor(sample["mano_betas"], dtype=torch.float32).unsqueeze(0)  # 손 모양(shape)
...
def cluster_stats(hand_mesh, obj_mesh, finger, contact, sd, tri_id, g_cam,
  ...
  for fi, fname in enumerate(_FINGER_NAMES):
    ...
    clusters.append({
        ...
        "max_penetration_mm": sd[sel].max() * 1000.0,   # 최대 관통 깊이(annotation 품질)
        "centroid": hand_mesh.vertices[sel].mean(axis=0),  # cluster 중심(torque용 r_k 후보)
    })
  return clusters
...
def main():
  ...
  parser.add_argument("--idx", type=int, default=421470)     # 확정 scene (Research context)
